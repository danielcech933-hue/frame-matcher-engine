...
import { lessonsQuery, LEVELS } from "@/lib/content";
...
function Admin() {
  const { data: userData } = useQuery({ queryKey: ["admin-user"], queryFn: () => supabase.auth.getUser() });
  const userId = userData.user?.id;
  const admin = useIsAdmin(userId);
  const qc = useQueryClient();
  const lessons = useQuery(lessonsQuery);
  const [editing, setEditing] = useState<string | null>(null);
  const [draft, setDraft] = useState({ slug: "", title: "", summary: "", content: "", level: "zacatecnik", duration_min: 10, published: true });

  const save = useMutation({
    mutationFn: async () => {
      if (!draft.slug.trim() || !draft.title.trim()) throw new Error("Slug a název jsou povinné.");
      const payload = { ...draft, duration_min: Number(draft.duration_min) || 10 };
      const query = editing ? supabase.from("lessons").update(payload).eq("slug", editing) : supabase.from("lessons").insert(payload);
      const { error } = await query;
      if (error) throw error;
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["lessons"] }); setEditing(null); setDraft({ slug: "", title: "", summary: "", content: "", level: "zacatecnik", duration_min: 10, published: true }); toast.success("Lekce uložena"); },
  ...
  const startEdit = (l: any) => { setEditing(l.slug); setDraft({ slug: l.slug, title: l.title, summary: l.summary, content: l.content, level: l.level, duration_min: l.duration_min, published: l.published }); window.scrollTo({ top: 0, behavior: "smooth" }); };
