...
function Overview() {
  ...
  const progress = useQuery({
    ...
    enabled: !!userId,
    queryFn: async () => { const { data, error } = await supabase.from("user_progress").select("*").eq("user_id", userId!); if (error) throw error; return data; },
  });
  const bookmarks = useQuery({
    queryKey: ["bookmarks", userId],
    enabled: !!userId,
    queryFn: async () => { const { data, error } = await supabase.from("bookmarks").select("*").eq("user_id", userId!).order("created_at", { ascending: false }); if (error) throw error; return data; },
  });
  ...
  return <SiteLayout><div className="mx-auto max-w-6xl px-4 py-12">
    <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between"><div><Badge variant="secondary">Můj účet</Badge><h1 className="mt-3 font-display text-3xl font-bold md:text-4xl">Váš učební přehled</h1><p className="mt-2 text-muted-foreground">{profile.data?.display_name ?? "Trader"}, tady vidíte svůj postup a uložené lekce.</p></div>{nextLesson && <Link to="/lekce/$slug" params={{ slug: nextLesson.slug }} className="inline-flex items-center gap-2 text-sm font-medium text-primary hover:underline">Pokračovat ve studiu <ArrowRight className="size-4" /></Link>}</div>
    <div className="mt-8 grid gap-4 md:grid-cols-4"><div className="surface p-5"><GraduationCap className="size-5 text-primary" /><p className="mt-3 text-sm text-muted-foreground">Pokrok</p><p className="font-display text-3xl font-bold">{percentage}%</p></div><div className="surface p-5"><CheckCircle2 className="size-5 text-primary" /><p className="mt-3 text-sm text-muted-foreground">Dokončeno</p><p className="font-display text-3xl font-bold">{completed.size}</p></div><div className="surface p-5"><Bookmark className="size-5 text-primary" /><p className="mt-3 text-sm text-muted-foreground">Záložky</p><p className="font-display text-3xl font-bold">{bookmarks.data?.length ?? 0}</p></div><div className="surface p-5"><Award className="size-5 text-primary" /><p className="mt-3 text-sm text-muted-foreground">Úroveň</p><p className="font-display text-xl font-bold">{profile.data ? levelLabel(profile.data.level) : "—"}</p></div></div>
